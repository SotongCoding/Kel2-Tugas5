...
namespace TankU.Unit.UnitAction
{
    public class UnitActionControl
    {
        ...
        public void Move()
        {
            ...
            if (KeyAction._moveUp) { thisUnit.transform.Translate(thisUnit.head.transform.forward * unitStatus._unitSpeed * Time.deltaTime); dir = thisUnit.head.transform.forward; }
            else if (KeyAction._moveDown) { thisUnit.transform.Translate(-thisUnit.head.transform.forward * unitStatus._unitSpeed * Time.deltaTime); dir = -thisUnit.head.transform.forward; }

            else if (KeyAction._moveLeft) { thisUnit.transform.Translate(-thisUnit.head.transform.right * unitStatus._unitSpeed * Time.deltaTime); dir = -thisUnit.head.transform.right; }
            else if (KeyAction._moveRight) { thisUnit.transform.Translate(thisUnit.head.transform.right * unitStatus._unitSpeed * Time.deltaTime); dir = thisUnit.head.transform.right; }
